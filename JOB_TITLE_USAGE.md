# JobTitle Model - Brugseksempler

## 📋 Oversigt

JobTitle modellen bruges til at normalisere og kategorisere job titler på tværs af alle lønsedler. Dette gør det muligt at sammenligne lønninger for samme stillings type.
...
### Payslip Relation
- `job_title_id` (foreign key) - Reference til JobTitle
- Relation: `belongsTo(JobTitle::class)`

## 💾 Arbejde med JobTitle
...
// Direkte oprettelse
$jobTitle = JobTitle::create(['name' => 'Data Scientist']);

// Find eksisterende
$jobTitle = JobTitle::where('name', 'Læge')->first();
```
...
$payslip->jobTitle()->associate($jobTitle);
$payslip->save();
...
$payslips = $jobTitle->payslips;

// Med eager loading
$payslips = $jobTitle->payslips()->with('media')->get();

// Gennemsnitsløn for job titel
$avgSalary = $jobTitle->payslips()
    ->whereNotNull('salary')
    ->avg('salary');

echo "Gennemsnitsløn for {$jobTitle->name}: " . number_format($avgSalary, 2, ',', '.') . " DKK";
```
...
$payslip = Payslip::find(1);
$extractor = new JobTitleExtractor();

// Ekstrahér job titel
$jobTitle = $extractor->extractJobTitle($payslip);

if ($jobTitle) {
    echo "Job titel: {$jobTitle->name}";
}
...
$topJobTitles = JobTitle::withCount('payslips')
    ->orderBy('payslips_count', 'desc')
    ->take(10)
    ->get();

foreach ($topJobTitles as $jobTitle) {
    echo "{$jobTitle->name}: {$jobTitle->payslips_count} lønsedler\n";
}
```
...
```php
$salaryStats = JobTitle::with('payslips')
    ->get()
    ->map(function ($jobTitle) {
        ...
        return [
            ...
            'count' => $jobTitle->payslips()->whereNotNull('salary')->count(),
        ];
    })
...
$highPayingJobs = JobTitle::whereHas('payslips', function ($query) {
    $query->where('salary', '>=', 50000);
})->get();

$entryLevelJobs = JobTitle::whereHas('payslips', function ($query) {
...
$salaryByExperience = $jobTitle->payslips()
    ->whereNotNull('salary')
...
```php
// Gennemsnitsløn per job titel
$avgSalaries = JobTitle::with('payslips')
    ->get()
    ->map(function ($jobTitle) {
        return [
            'name' => $jobTitle->name,
            'avg_salary' => $jobTitle->payslips()->avg('salary'),
            'count' => $jobTitle->payslips()->count(),
        ];
    })
    ->sortByDesc('avg_salary');
```

## 📈 Fordele ved JobTitle Model
...
Payslip::whereNull('job_title_id')
    ->whereNotNull('title')
    ->chunk(100, function ($payslips) {
        foreach ($payslips as $payslip) {
            $extractor = new \App\Services\JobTitleExtractor();
...
foreach ($mapping as $from => $to) {
    $fromJob = JobTitle::where('name', $from)->first();
    $toJob = JobTitle::firstOrCreate(['name' => $to]);
